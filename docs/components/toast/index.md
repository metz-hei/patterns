...
[Предыдущая версия v.1 в Figma](https://www.figma.com/design/bYUKHrjBAhYPLdug8qfzFc/09-%E2%9C%85-Popup?node-id=503-5047)
...
Если тост устаревает до закрытия, то он должен автоматически смениться на новый.

Если тост с прелоадером заменяется на результирующий, то смена контента происходит мгновенно без анимации.

Например, после восстановления интернет-соединения тост с прелоадером заменяется на успешное состояние. Запускается таймер отсчёта для успешного тоста — 3 сек.
![Замена устаревшего тоста](./toast-03guideline-03queue-03replacement.png)

Если важно, чтобы пользователь заметил смену тоста (меняется статус и/или текст), то нужно сначала скрыть старый тост, затем показать новый.